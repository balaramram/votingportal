...
const AdminDashboard = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch("https://votingportal-tv3f.onrender.com/api/voting/create", {
        method: "POST",
        headers: { 
  ...
  const handleDeletePoll = async (pollId) => {
    if (window.confirm("Are you sure you want to delete this poll? 🗑️")) {
      try {
        const token = localStorage.getItem("adminToken");
        const res = await fetch(`https://votingportal-tv3f.onrender.com/api/voting/delete-poll/${pollId}`, {
          method: "DELETE",
